...
from my_config import dir_local, dir_ssd_path_population

# this is needed to avoid the error in the download in MaxOS
os.environ["no_proxy"] = "*"

data_population_path = dir_local / "population"
    ...
    for year in years_range:
        for sex in ["m", "f"]:
            for age in [0, 65, 70, 75, 80]:
                ...
                filepath = dir_local / f"population/global_{sex}_{age}_{year}_1km.tif"
                tmp_filepath = tmp_path / filepath.name
                hd_filepath = dir_ssd_path_population / filepath.name
                if (
...
def create_urls_aggregated_years() -> list[tuple[str, str]]:
    ...
    for year in years_range:
        ...
        filepath = dir_local / f"population/ppp_{year}_1km_Aggregated.tif"
        tmp_filepath = tmp_path / filepath.name
        if not filepath.is_file() and not tmp_filepath.is_file():
            urls.append((download_url, filepath))
    return urls
